...
// src/enviroment.ts
var CronosZkEVMEnvSchema = z.object({
...
async function validateCronosZkevmConfig(runtime) {
  try {
    const config = {
      CRONOSZKEVM_ADDRESS: runtime.getSetting("CRONOSZKEVM_ADDRESS") || process.env.CRONOSZKEVM_ADDRESS,
      CRONOSZKEVM_PRIVATE_KEY: runtime.getSetting("CRONOSZKEVM_PRIVATE_KEY") || process.env.CRONOSZKEVM_PRIVATE_KEY
    };
...
var transfer_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      if (callback) {
        callback({
          text: "Transfer completed successfully! tx: " + receipt.transactionHash,
          content: {}
        });
      }
      return true;
...
var index_default = cronosZkEVMPlugin;
export {
  cronosZkEVMPlugin,
