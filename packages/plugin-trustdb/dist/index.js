...
var TrustScoreDatabase = class {
  ...
  getRecommender(identifier) {
    const sql = `
        ...
        `;
    const recommender = this.db.prepare(sql).get(
      identifier,
      identifier,
      identifier,
      identifier,
  ...
  upsertTokenPerformance(performance) {
    ...
    try {
      this.db.prepare(sql).run(
        ...
        // Ensure column name matches schema
  ...
  addTokenRecommendation(recommendation) {
    ...
    try {
      this.db.prepare(sql).run(
        ...
        recommendation.timestamp || /* @__PURE__ */ new Date(),
  ...
  getRecommendationsByToken(tokenAddress) {
    ...
    return rows.map((row) => ({
      ...
      tokenAddress: row.token_address,
      timestamp: new Date(row.timestamp),
      initialMarketCap: row.initial_market_cap ?? void 0,
      initialLiquidity: row.initial_liquidity ?? void 0,
      initialPrice: row.initial_price ?? void 0
    }));
  }
  /**
  ...
  getRecommendationsByDateRange(startDate, endDate) {
    const sql = `
        ...
        `;
    const rows = this.db.prepare(sql).all(startDate.toISOString(), endDate.toISOString());
    return rows.map((row) => ({
      id: row.id,
      recommenderId: row.recommender_id,
      tokenAddress: row.token_address,
  ...
  `;
    try {
      this.db.prepare(sql).run(
        ...
        trade.last_updated || (/* @__PURE__ */ new Date()).toISOString(),
        trade.rapidDump ? 1 : 0
      );
  ...
  getTradePerformance(tokenAddress, recommenderId, buyTimeStamp, isSimulation) {
    ...
    const row = this.db.prepare(sql).get(tokenAddress, recommenderId, buyTimeStamp);
    if (!row) return null;
    return {
