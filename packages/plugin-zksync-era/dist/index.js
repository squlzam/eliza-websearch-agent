...
// src/enviroment.ts
var zksyncEnvSchema = z.object({
  ZKSYNC_ADDRESS: z.string().min(1, "ZKsync address is required"),
  ZKSYNC_PRIVATE_KEY: z.string().min(1, "ZKsync private key is required")
});
async function validateZKsyncConfig(runtime) {
  try {
    const config = {
      ZKSYNC_ADDRESS: runtime.getSetting("ZKSYNC_ADDRESS") || process.env.ZKSYNC_ADDRESS,
      ZKSYNC_PRIVATE_KEY: runtime.getSetting("ZKSYNC_PRIVATE_KEY") || process.env.ZKSYNC_PRIVATE_KEY
    };
...
var index_default = zksyncEraPlugin;
export {
